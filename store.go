...
type buckets map[string]map[string]interface{}
...
func newStore(base string, valid func(string) bool) store {
	return store{base: base, dirty: false, valid: valid, meta: make(buckets)}
}

func prepare(s store, path string) (map[string]interface{}, error) {
	path = Clean(path)
	if s.valid != nil && !s.valid(path) {
...
// I am trying to do this: https://play.golang.org/p/Bv7JssF99K it does not work.
// see https://play.golang.org/p/bLJuwdbvMG for working code using reflection
// see also https://play.golang.org/p/mawvuoGUfP
// not quite as helpful but interesting https://play.golang.org/p/73lJyOlaIdL
...
func (s store) Get(path, key string, valuePtr interface{}) (ok bool, err error) {
	...
	value, ok := b[key] // get the value
	if !ok {
		return false, nil
	}

	typ := reflect.TypeOf(valuePtr)
	if typ.Kind() != reflect.Ptr {
		return false, ErrNeedPointer
	}
